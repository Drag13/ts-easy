...
Правда решить это не сложно. Мы можем просто связать тип который возвращает маппер и тип который мы ожидаем от метода fetch:
...
Теперь посмотрим на сигнатуру функции `fetchApi`. Вы помните структуру options? Я - только частично. Хотелось бы что бы TS нам и тут помогал, но описывать весь тип вручную явно противоречит идеи простоты. К счастью, мы можем просто переиспользовать уже существующие типы:
